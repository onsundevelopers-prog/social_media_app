...
const SignupForm = () => {
  ...
  const handleSignup = async (user: z.infer<typeof SignupValidation>) => {
    try {
      ...
      if (!newUser) return toast({ title: "Sign up failed." });

      const session = await signInAccount({ email: user.email, password: user.password });
      if (!session) return toast({ title: "Login failed. Try signing in." });
  ...
  return (
    <Form {...form}>
      <div className="max-w-feed flex-center flex-col">
        <img src="/assets/images/logo.svg" alt="logo" />

        <h2 className="h3-bold md:h2-bold pt-5 sm:pt-12 text-text">
          Create a new account
        </h2>
        <p className="text-muted small-medium md:base-regular mt-2 text-center">
